...
var timer
var runIt
var theTimer = document.getElementById('theTimer') //this variable calls out to the id element, theTimer
...
var counter = 1 //this variable is the counter and it was set at 0. It did not make sense to me for the counter to go from 00 to 0 to 1, so I placed the counter to begin at 1. Now, the timer goes from 00 to 1. The counter must be a variable so it can be used within functions.
...
function resetFunction() { //the reset function sets the actions for setting the reset button. It sets the counter back to 0
    clearInterval(timer)
    counter = 0
    theTimer.innerHTML = counter
    startThis.innerHTML = 'Start'

}

function startClock () { //this consistently and incrementally runs the clock after starting
    theTimer.innerHTML = counter //theTimer is listed as the ID of the div that we are displaying the content within
